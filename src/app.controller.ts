...
import { CategoryDTO, ProductDTO, SignInDTO, SignUpDTO } from './dto';
...
export class AppController {
  ...
  async createCategory(@Body() category:CategoryDTO, @Res() res: Response):Promise< any> {
    const result = await this.appService.createCategory(category);
    return res.status(200).json(result);
  }

  @Post('create-product')
  async createProduct(@Body() product:ProductDTO,@Req() req:Request, @Res() res: Response):Promise< any> {
